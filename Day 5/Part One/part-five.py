...
def move_boxes(box_list, count, from_index, to_index):
    for i in range(0,count):
        print(f'├ Step #{i+1}'.ljust(150, " "), end="│\n")
        print(f'├──── [{from_index}] : {box_list[from_index-1]}'.ljust(150, " "), end="|\n")

        item = box_list[from_index-1].pop()
        
        print(f'├──── {item} -> column {to_index} : {box_list[to_index-1]}'.ljust(150, " "), end="|\n")

        box_list[to_index-1].append(item)

        print(f'├──── [{to_index}] : {box_list[to_index-1]}'.ljust(150, " "), end="|\n")
